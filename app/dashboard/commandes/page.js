...
export default function Commandes() {
  ...
  async function fetchData() {
    setLoading(true);
    try { const r = await api('GET', {}, { action: 'commandes', page: String(page), search }); setData(r); } catch {}
    setLoading(false);
  }
        ...
        {loading ? <div className="adld"><div className="adsp" /></div> : (
          <div className="adtw">
            <table className="adtabl">
              <thead><tr><th>ID</th><th>Montant</th><th>Statut</th><th>Date</th><th>Actions</th></tr></thead>
              <tbody>
                {(data.commandes||[]).map(c => (
                  <tr key={c.id}>
                    <td style={{fontSize:'.72rem',color:'#666',fontFamily:'monospace'}}>{c.id?.slice(0,12)}</td>
        ...
        <div className="admb" onClick={e=>{if(e.target===e.currentTarget)setModal(null)}}>
          <div className="admcont">
            ...
            <div className="admbody">
              ...
              <p style={{fontSize:'.85rem',color:'#666',lineHeight:1.6}}>Supprimer la commande <strong>{modal.item.id?.slice(0,12)}</strong> ?</p>
            </div>
